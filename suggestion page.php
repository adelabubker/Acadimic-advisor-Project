...
$student_info_query = "SELECT years, GPA FROM student WHERE std_id = $student_id";
...
$sql = "SELECT hour_complete FROM student WHERE std_id = $student_id";
$result = $con->query($sql);
$row = $result->fetch_assoc();
$hour_complete = $row['hour_complete'];
...
// معالجة الساعات المطلوبة من المستخدم

if (isset($_POST['hours']) && is_numeric($_POST['hours'])) {
    $desired_hours = $_POST['hours'];  
} else {
    $desired_hours = 0;
}
$suggested_courses_query = "
    SELECT c.course_id, c.course_name, c.num_hour, c.evaluation, c.academic_year, c.have_prerequisite, c.Open_study_sub,
           c.success_rate,
           CASE 
           ...
           CASE 
               WHEN c.Open_study_sub = 'open' AND $student_gpa >= 70 THEN 'احتمالية عالية'
               WHEN c.Open_study_sub = 'open' AND $student_gpa >= 50 THEN 'احتمالية متوسطة'
               ELSE 'احتمالية منخفضة'
           END AS probability
    ...
    AND (
        ...
        (c.evaluation = 'year1 less' AND $student_year >= 1 AND $hour_complete > 9) OR 
        (c.evaluation = 'year1 more' AND $student_year >= 1 AND $hour_complete > 13) OR 
        ...
        (c.evaluation = 'year4' AND $student_year >= 4 AND $hour_complete > 92)
    )
    ...
    CASE 
        ...
        WHEN c.Open_study_sub = 'open' THEN 6
        WHEN c.Open_study_sub = 'Do not open' THEN 7
        ELSE 8
    END,
...
<button style="float: left;" type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  ...
  <strong style="float: right;color:red">: تنبيه هام <i class="bi bi-exclamation-triangle-fill"></i></strong > (Have a prerequisite) يجب معرفة المواد التي له متطلب سابق قبل  الاختيار من المواد المقترحة
, <strong>في جدول الاقتراحات انظر لعامود</strong>
    ...
    <form  style="margin-top: 70px;"  method="post">
        ...
        <button id="btn_ok" type="submit" class="btn" onclick="document.location='#suggested_courses'">OK</button>
    </form>
    ...
    <table  class="table table-bordered-dark mt-3">
        <thead >
            <tr>
                ...
                <th style="color: black;">Success probability</th>
            </tr>
        </thead>
        <tbody>
            <?php
...
<footer class="footer">
    <div class="container text-center">
        <p>Copyrights @2024 by: <span style="color: white;">Mohammed Albadawi</span></p>
        <div class="row">
            <div class="col">
